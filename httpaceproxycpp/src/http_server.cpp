...
bool parse_http_request(int fd, const std::string& client_ip, HttpRequest& request) {
    ...
    std::string initial_body = raw.substr(body_start);
    auto cl_str = request.header("content-length");
    if (!cl_str.empty()) {
        std::size_t cl = 0;
        try { cl = std::stoul(cl_str); } catch (...) {}
        request.body = initial_body;
        char tmp[8192];
        while (request.body.size() < cl) {
            ssize_t n = ::recv(fd, tmp, std::min(sizeof(tmp), cl - request.body.size()), 0);
            if (n <= 0) break;
